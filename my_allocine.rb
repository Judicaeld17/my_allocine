...
requests["Find the name of movie and director who directed a movies that casted a role as Sean Maguire"] = "SELECT dir_fname, dir_lname, mov_title FROM movies INNER JOIN movies_actors ON movies.id = movies_actors.mov_id INNER JOIN directors_movies ON movies_actors.mov_id = directors_movies.mov_id INNER JOIN directors ON directors_movies.dir_id = directors.id WHERE role = 'Sean Maguire';"
requests["Find all the actors who have not acted in any movie between 1990 and 2000 (select only actor first name, last name, movie title and release year)"] = "SELECT act_fname,act_lname,mov_title,CAST(strftime('%Y',mov_dt_rel) AS INTEGER) FROM movies INNER JOIN movies_actors ON movies.id = movies_actors.mov_id INNER JOIN actors ON actors.id =movies_actors.act_id WHERE mov_dt_rel NOT BETWEEN '1990-01-01' AND '2000-01-01' LIMIT 14;"